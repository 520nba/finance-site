/**
 * 统一存储抽象层 (极简 KV 版)
 * 彻底废弃数据库，全量依赖 Cloudflare KV
 */

import { getRequestContext, getCloudflareContext } from "@opennextjs/cloudflare";

// 内存缓存 fallback (仅限本地开发且无 KV 绑定时)
const memoryCache = new Map();

async function getKvStorage() {
    // 方案 1: getRequestContext
    try {
        const ctx = getRequestContext();
        if (ctx?.env?.STOCK_DATA) return ctx.env.STOCK_DATA;
    } catch (e) {
        console.log('[Storage] getRequestContext error:', e.message);
    }

    // 方案 2: getCloudflareContext (部分版本可用)
    try {
