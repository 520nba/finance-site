...
async function getKvStorage() {
    // 方案 1: getCloudflareContext (目前版本的主流方式)
    try {
        if (Cloudflare.getCloudflareContext) {
            const ctx = await Cloudflare.getCloudflareContext();
    ...
    // 方案 2: getRequestContext (部分旧版本或特殊配置)
    try {
        if (Cloudflare.getRequestContext) {
            const ctx = Cloudflare.getRequestContext();
            if (ctx?.env?.STOCK_DATA) return ctx.env.STOCK_DATA;
        }
    } catch (e) { }

    // console.warn('[Storage] All KV binding methods failed. Falling back to memoryCache.');
    return null;
}
