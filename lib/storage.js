/**
 * 统一存储抽象层 (极简 KV 版)
 * 彻底废弃 D1 数据库，全量迁移至 Cloudflare KV
 */
...
export async function getHistoryFromDB(code, type, days = 250) {
    const key = `hist:${type}:${code}`;
    const history = await readDoc(key, []);
    // 返回最近的 days 条，并确保按日期升序 (KV 存储建议直接存有序数组)
    return history.slice(-days);
}

export async function getBulkHistoryFromDB(items, days = 250) {
    const kv = await getKvStorage();
    if (!kv) return {};
...
export async function insertDailyPricesBatch(records) {
    ...
    const promises = Object.entries(groups).map(async ([key, items]) => {
        ...
        const history = await readDoc(kvKey, []);
...
export async function getBulkIntradayFromDB(items, date) {
    ...
    const result = {};
    const promises = items.map(async (it) => {
        const data = await kv.get(`intra:${it.code}:${date}`);
...
export async function getAssetNamesFromDB(items) {
    ...
    const promises = items.map(async (it) => {
        const name = await kv.get(`name:${it.type}:${it.code}`);
...
export async function getBulkQuotesFromDB(codes) {
    ...
    const promises = codes.map(async (code) => {
        const data = await kv.get(`quote:${code}`);
