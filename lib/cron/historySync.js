...
const BATCH_DELAY_MS = 200;   // 批间等待（ms）- 适当延长以降低限流风险
const SUBREQ_BUDGET = 40;    // 统一配额管理：D1 Free 计划硬上限 50，保留 10 次余量
const FETCH_TIMEOUT = 10000; // 单资产拉取超时（ms）- 稍微放宽
...
async function _syncOne(env, asset, force = false) {
    ...
    // 4. 如果是强制刷新，先物理删除旧记录，确保不残留 API 已剔除的日期
    if (force) {
        await env.DB.prepare('DELETE FROM asset_history WHERE code = ? AND type = ?')
            .bind(code.toLowerCase(), type)
            .run();
        consumed += 1;
    }
