...
export async function fetchIntradayData(code, type) {
    ...
    try {
        // 尝试 trends/get 接口
        const url = `https://push2.eastmoney.com/api/qt/stock/trends/get?secid=${market}.${clean}&fields1=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11&fields2=f51,f52,f53,f54,f55,f56,f57,f58`;
        const res = await fetchWithProxy(url);
        if (!res.ok) return null;
        const json = await res.json();
        const d = json.data;
        ...
        const prevClose = (d.prePrice ?? d.preClose) / 100;
        const points = d.trends.map(line => {
            const parts = line.split(',');
            // 时间处理兼容: "2024-02-28 09:30" 或 "09:30"
