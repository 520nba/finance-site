import { fetchWithRetry, getBeijingTodayStr, isMarketClosedBeijing } from '@/lib/utils';
...
export async function fetchFundHistory(code, days, signal = null) {
    ...
    try {
        ...
        // 转换数据：时间戳(ms) -> YYYY-MM-DD (强制北京时间)
        const points = rawData.map(item => {
            return {
                date: formatDateInTimezone(item[0], "Asia/Shanghai"),
