...
export async function fetchFundHistory(code, days) {
    ...
    try {
        ...
        // 1. 探测第一页获取总量
        const probeUrl = `https://api.fund.eastmoney.com/f10/lsjz?fundCode=${clean}&pageIndex=1&pageSize=${pageSize}&_=${Date.now()}`;
        ...
        // 3. 转换并返回 (只取前 targetCount 条，API 返回通常是按日期倒序的)
        const finalData = resultList
            .slice(0, targetCount)
            .map(i => ({
                date: i.FSRQ,
                value: parseFloat(i.LJJZ) // 强制使用累计净值
            }))
            .filter(i => !isNaN(i.value))
            .reverse(); // 转为时间轴升序

        await addSystemLog('INFO', 'ExternalAPI', `Fund: Fetched ${finalData.length} LJJZ points for fund ${code}`);
        return finalData;
    } catch (e) {
