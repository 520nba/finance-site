...
export async function fetchFundHistory(code, days) {
    ...
    try {
        const url = `https://api.fund.eastmoney.com/f10/lsjz?fundCode=${clean}&pageIndex=1&pageSize=300&_=${Date.now()}`
        const res = await fetchWithRetry(url, {
            headers: {
                ...BASE_HEADERS,
        ...
        if (list.length > 0) {
            const data = list
                ...
                .slice(0, days)
                .reverse();
            await addSystemLog('INFO', 'ExternalAPI', `Fund: Fetched ${data.length} accumulative points for fund ${code}`);
            return data;
        }
        return null
    } catch (e) {
