...
export async function fetchFundHistory(code, days) {
    ...
    try {
        ...
        // 2. 如果第一页不够，顺序抓取后续页面 (维度 1 优化：顺序抓取比并发更稳，防止触发 WAF)
        const targetCount = Math.min(days, totalCount);
        const totalPages = Math.ceil(targetCount / pageSize);

        if (totalPages > 1) {
            for (let p = 2; p <= totalPages; p++) {
                const uniqueTs = Date.now() + p;
                const pageRes = await fetchWithRetry(
                    `https://api.fund.eastmoney.com/f10/lsjz?fundCode=${clean}&pageIndex=${p}&pageSize=${pageSize}&_=${uniqueTs}`,
                    { headers: { ...BASE_HEADERS, 'Referer': 'http://fundf10.eastmoney.com/' } }
                );

                if (!pageRes) {
                    console.error(`[Fetcher] Fund ${code} page ${p} fetch failed. Aborting.`);
                    return null;
                }

                const pageData = await pageRes.json();
                if (!pageData?.Data?.LSJZList) {
                    console.error(`[Fetcher] Fund ${code} page ${p} missing data. Aborting.`);
                    return null;
                }

                resultList.push(...pageData.Data.LSJZList);
        ...
        // 4. 关键：数量校验 (维度 1 优化：如果缺漏超过 10%，则认为同步失败，不写入数据库)
        // 这样可以防止出现“断层”曲线。返回 null 会触发任务重试。
        const minExpected = Math.floor(targetCount * 0.95);
        if (converted.length < minExpected && targetCount > 50) {
            console.error(`[Fetcher] Fund ${code} data integrity check failed: Got ${converted.length}, expected ~${targetCount}. Aborting and retrying later.`);
            return null;
        }

        console.log(`[Fetcher] Fund: Fetched ${converted.length} LJJZ points for fund ${code}`);
        return converted;
    } catch (e) {
