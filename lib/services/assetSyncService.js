...
async function fetchStockName(code) {
    ...
    try {
        ...
        const res = await fetchWithRetry(url, EASTMONEY_HEADERS);
        if (res.ok) {
...
async function fetchFundName(code) {
    ...
    try {
        ...
        const res = await fetchWithRetry(url, TENCENT_HEADERS);
        if (res.ok) {
    ...
    try {
        ...
        const res = await fetchWithRetry(url, EASTMONEY_FUND_HEADERS);
        if (res.ok) {
...
export async function fetchSingleIntraday(code, preFetchedDbData = null, externalAbortSignal = null) {
    ...
    let res
    try {
        res = await fetchWithRetry(url, EASTMONEY_HEADERS, externalAbortSignal)
    } catch (e) {
