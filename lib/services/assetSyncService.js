...
export async function syncHistoryBulk(items, days = 250, allowExternal = false, env = null, forceRefresh = false) {
    ...
    const dbHistoryMap = (forceRefresh) ? {} : await getBulkHistory(items, days, env)
    const result = {}
    const today = getBeijingTodayStr()
    ...
    for (const item of items) {
        ...
        // 数据完整性判定逻辑
        const latestDateInDb = dbHistory.length > 0 ? dbHistory[dbHistory.length - 1].date : null
        const gap = latestDateInDb ? daysBetween(latestDateInDb, today) : 999

        // 扩充同步判定条件：允许外部抓取 且 (强制刷新 或 数据库为空 或 数据有滞后)
        const shouldFetch = allowExternal && (forceRefresh || dbHistory.length === 0 || gap > 0)

        if (shouldFetch) {
            try {
                const { fetchStockHistory, fetchFundHistory } = await import('./historyFetcher')
                const externalData = item.type === 'fund'
                ...
                if (externalData?.length > 0) {
                    ...
                    if (forceRefresh) {
                        try {
                            ...
                            console.warn(`[SyncBulk] Force cleanup failed for ${key}:`, delErr.message)
                        }
                    }
                    ...
                    try {
                        await insertDailyPricesBatch(records, env)
                        dbHistory = externalData
                        console.log(`[SyncBulk] ${key}: Successfully synced ${externalData.length} records.`)
                    } catch (dbErr) {
                        console.error(`[SyncBulk] DB writer failed for ${key}:`, dbErr.message)
                    }
                } else {
                    console.log(`[SyncBulk] ${key}: No data fetched from external source.`)
                }
            } catch (e) {
                console.error(`[SyncBulk] Process failed for ${key}:`, e.message)
            }
        }
