...
export async function syncHistoryBulk(items, days = 250, allowExternal = false, env = null) {
    ...
    for (const item of items) {
        ...
        if (dbHistory.length === 0 && allowExternal) {
            try {
                const { fetchHistorySingle } = await import('./historyFetcher')
                const { saveHistory } = await import('../storage/historyRepo')

                const externalData = await fetchHistorySingle(item.code, item.type, days)
                if (externalData && externalData.length > 0) {
                    await saveHistory(item.code, item.type, externalData)
