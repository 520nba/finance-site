...
export async function syncHistoryBulk(items, days = 250, allowExternal = false, env = null, force = false) {
    if (!items?.length) return {}
    const dbHistoryMap = (force) ? {} : await getBulkHistory(items, days, env)
    const result = {}
    const today = getBeijingTodayStr()
    ...
    for (const item of items) {
        ...
        // 如果 D1 没数据（或由于 force 为 true 导致 dbHistoryMap 为空）且允许外部抓取
