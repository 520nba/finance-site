 ...
 * @param {Object} env
 */
export async function addSystemLog(level, module, message, env = null) {
    ...
    try {
        if (shouldPersist) {
            await runSql(
                'INSERT INTO system_logs (level, module, message, timestamp) VALUES (?, ?, ?, datetime("now", "+8 hours"))',
 ...
 * @param {Object} env
 */
export async function getSystemLogs(hours = 72, env = null) {
    try {
        return await queryAll(
            'SELECT * FROM system_logs WHERE timestamp >= datetime("now", "+8 hours", "-" || ? || " hours") ORDER BY timestamp DESC',
            [hours],
            env
        );
    } catch (e) {
