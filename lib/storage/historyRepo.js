    ...
    const sql = `
        ...
        WHERE rowid IN (
            ...
            WHERE LOWER(status) = 'pending'
               OR (LOWER(status) = 'error' AND updated_at < datetime('now', '-1 hour'))
               OR (LOWER(status) = 'syncing' AND updated_at < datetime('now', '-10 minutes'))
            ORDER BY 
                CASE status 
                    WHEN 'pending' THEN 0 
                    WHEN 'syncing' THEN 1 
...
export async function updateSyncStatus(code, type, status) {
    if (status === 'done') {
        return await runSql('DELETE FROM sync_queue WHERE LOWER(code) = LOWER(?) AND type = ?', [code, type]);
    }
    return await runSql(
        'UPDATE sync_queue SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE LOWER(code) = LOWER(?) AND type = ?',
        [status, code, type]
    );
}
...
export async function insertDailyPricesBatch(records) {
    ...
    for (let i = 0; i < records.length; i += 50) {
        ...
        try {
            ...
            if (!results) throw new Error('D1 batch returned null');
        } catch (e) {
...
export async function incrementalUpdateFundHistory(code, records) {
    ...
    const cleanupSql = `
        ...
        WHERE LOWER(code) = LOWER(?) AND type = 'fund' 
        AND record_date < (
            SELECT record_date 
            FROM asset_history 
            WHERE LOWER(code) = LOWER(?) AND type = 'fund' 
    ...
    for (let i = 0; i < items.length; i += 50) {
        ...
        const stmts = chunk.map(item =>
            db.prepare(`
                ...
                FROM asset_history 
                WHERE LOWER(code) = LOWER(?) AND type = ? 
...
export async function getHistory(code, type, days = 250) {
    const querySql = `
        ...
        ORDER BY record_date DESC LIMIT ?
    `;
    const rows = await queryAll(querySql, [code, type, days]);
