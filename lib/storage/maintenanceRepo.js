...
export async function cleanupSingleAssetIfNotUsed(type, code) {
    try {
        // 在 D1 架构下，JOIN/EXISTS 查询是实时的，移除旧 KV 时代的 3s 宽限期延迟
        // 从而实现用户要求的“马上删除”反馈。
        ...
        if (!row) {
            ...
            await db.batch([
                ...
                db.prepare('DELETE FROM asset_names WHERE code = ? AND type = ?').bind(code, type),
                db.prepare('DELETE FROM asset_intraday WHERE code = ?').bind(code),
                db.prepare('DELETE FROM asset_quotes WHERE code = ?').bind(code)
            ]);
    ...
    try {
        ...
        const resHist = await db.prepare(`
            ...
            WHERE NOT EXISTS (
                ...
                WHERE user_assets.code = asset_history.code 
                AND user_assets.type = asset_history.type
            )
        `).run();
        ...
        const resNames = await db.prepare(`
            ...
            WHERE NOT EXISTS (
                SELECT 1 FROM user_assets 
                WHERE user_assets.code = asset_names.code 
                AND user_assets.type = asset_names.type
            )
        `).run();
